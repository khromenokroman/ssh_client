...
SshClient::SshClient(std::string_view host, int port, std::string_view username, std::string_view password)
    ...
    libssh2_session_set_blocking(m_session.get(), 1);
}
std::string SshClient::execute(std::string_view command) {
    ...
    if (libssh2_channel_exec(channel.get(), std::string(command).c_str()) != 0) {
    ...
    // Читаем stdout и stderr до EOF канала.
    while (true) {
        // stdout
        ssize_t readBytes = libssh2_channel_read_ex(channel.get(), 0, buffer.data(), buffer.size());
        if (readBytes > 0) {
            output.append(buffer.data(), static_cast<size_t>(readBytes));
            continue;
        }
        if (readBytes < 0 && readBytes != LIBSSH2_ERROR_EAGAIN) {
            throw std::runtime_error("Channel stdout read failed");
        }

        // stderr
        ssize_t errBytes = libssh2_channel_read_ex(channel.get(), SSH_EXTENDED_DATA_STDERR,
                                                   buffer.data(), buffer.size());
        if (errBytes > 0) {
            output.append(buffer.data(), static_cast<size_t>(errBytes));
            continue;
        }
        if (errBytes < 0 && errBytes != LIBSSH2_ERROR_EAGAIN) {
            throw std::runtime_error("Channel stderr read failed");
        }

        if (libssh2_channel_eof(channel.get()) == 1) {
            break;
        }
    }
